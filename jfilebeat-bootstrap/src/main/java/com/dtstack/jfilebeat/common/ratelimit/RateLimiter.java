...
import com.dtstack.jfilebeat.common.ratelimit.SmoothRateLimiter.SmoothBursty;
import com.dtstack.jfilebeat.common.ratelimit.SmoothRateLimiter.SmoothWarmingUp;
 ...
 * <a href="http://en.wikipedia.org/wiki/Little%27s_law">Little's Law</a>).
 *
 * <p>
 ...
 * affects the throttling of the request itself (an invocation to
...
public abstract class RateLimiter {
	 ...
	 * exceeds {@code permitsPerSecond} the rate limiter will release one permit
	 * every {@code
	 * (1.0 / permitsPerSecond)} seconds. When the rate limiter is unused, bursts of
	 * up to {@code permitsPerSecond} permits will be allowed, with subsequent
	 * requests being smoothly limited at the stable rate of
	 * {@code permitsPerSecond}.
	 *
	 * @param permitsPerSecond
	 *            the rate of the returned {@code RateLimiter}, measured in how many
	...
	public static RateLimiter create(double permitsPerSecond) {
		 ...
		 * one second. This is to avoid unnecessary stalls in situations like this: A
		 * RateLimiter of 1qps, and 4 threads, all calling acquire() at these moments:
		 *
		 ...
		 * T3 would also have to sleep till 3.05 seconds.
		 */
		return create(SleepingStopwatch.createFromSystemTimer(), permitsPerSecond);
	}

	/*
	 * TODO(cpovirk): make SleepingStopwatch the last parameter throughout the class
	 * so that the overloads follow the usual convention: Foo(int), Foo(int,
	 * SleepingStopwatch)
	 */
	 ...
	 * the period (as long as there are enough requests to saturate it). Similarly,
	 * if the {@code RateLimiter} is left <i>unused</i> for a duration of
	 * {@code warmupPeriod}, it will gradually return to its "cold" state, i.e. it
	 * will go through the same warming up process as when it was first created.
	 *
	 * <p>
	 * The returned {@code RateLimiter} is intended for cases where the resource
	 * that actually fulfills the requests (e.g., a remote server) needs "warmup"
	 ...
	 * period will follow), and if it is left unused for long enough, it will return
	 * to that state.
	 *
	 * @param permitsPerSecond
	 *            the rate of the returned {@code RateLimiter}, measured in how many
	 *            permits become available per second
	 * @param warmupPeriod
	 *            the duration of the period where the {@code RateLimiter} ramps up
	 *            its rate, before reaching its stable (maximum) rate
	 * @param unit
	 *            the time unit of the warmupPeriod argument
	 * @throws IllegalArgumentException
	 *             if {@code permitsPerSecond} is negative or zero or
	 ...
	 * <b>not</b> be awakened as a result of this invocation, thus they do not
	 * observe the new rate; only subsequent requests will.
	 *
	 * <p>
	 * Note though that, since each request repays (by waiting, if necessary) the
	 * cost of the <i>previous</i> request, this means that the very next request
	 ...
	 * seconds, it still has a warmup period of 20 seconds after this method
	 * invocation.
	 *
	 * @param permitsPerSecond
	 *            the new stable rate of this {@code RateLimiter}
	 * @throws IllegalArgumentException
	 *             if {@code permitsPerSecond} is negative or zero
	 */
	 ...
	 * {@code RateLimiter} is configured with. The initial value of this is the same
	 * as the {@code permitsPerSecond} argument passed in the factory method that
	 * produced this {@code RateLimiter}, and it is only updated after invocations
	 ...
	 *         rate-limited
	 * @throws IllegalArgumentException
	 *             if the requested number of permits is negative or zero
	 * @since 16.0 (present in 13.0 with {@code void} return type})
	 */
	 ...
	 * use, returning the number of microseconds until the reservation can be
	 * consumed.
	 *
	 ...
	 * exceeding the specified {@code timeout}, or returns {@code false} immediately
	 * (without waiting) if the permit would not have been granted before the
	 * timeout expired.
	 *
	 * <p>
	 ...
	 *
	 * @param timeout
	 *            the maximum time to wait for the permit. Negative values are
	 *            treated as zero.
	 * @param unit
	 ...
	 * be obtained without exceeding the specified {@code timeout}, or returns
	 ...
	 *            the number of permits to acquire
	 * @param timeout
	 *            the maximum time to wait for the permits. Negative values are
	 *            treated as zero.
	 * @param unit
	 *            the time unit of the timeout argument
	 * @return {@code true} if the permits were acquired, {@code false} otherwise
	...
	abstract static class SleepingStopwatch {
		 ...
		 * Perhaps we need to guarantee that each call to reserveEarliestAvailable, etc.
		 * sees a value >= the previous? Also, is it OK that we don't hold the mutex
		 * when sleeping?
		 */
