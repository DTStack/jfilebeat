...
public final class Matcher implements MatchResult {
	...
	/**
	 * The storage used by groups. They may contain invalid values if a group was
	 * skipped during the matching.
	 */
	int[] groups;
	...
	/**
	 * Storage used by nodes to tell what repetition they are on in a pattern, and
	 * where groups begin. The nodes themselves are stateless, so they rely on this
	 * field to hold state during a match.
	 */
	int[] locals;
	...
	/**
	 * Changes the <tt>Pattern</tt> that this <tt>Matcher</tt> uses to find matches
	 * with.
	 *
	 * <p>
	 ...
	 * expression <i>m.</i><tt>start(0)</tt> is equivalent to
	 * <i>m.</i><tt>start()</tt>.
	 * </p>
	 *
	 * @param group
	 ...
	 * <a href="Pattern.html#cg">Capturing groups</a> are indexed from left to
	 * right, starting at one. Group zero denotes the entire pattern, so the
	 * expression <i>m.</i><tt>end(0)</tt> is equivalent to <i>m.</i><tt>end()</tt>.
	 * </p>
	 *
	 * @param group
	 *            The index of a capturing group in this matcher's pattern
	 ...
	 *         the previous match, or <tt>null</tt> if the group failed to match
	 *         part of the input
	 *
	 * @throws IllegalStateException
	 *             If no match has yet been attempted, or if the previous match
	...
	public Map<String, String> groupAsMap() {

		Set<String> keys = parentPattern.namedGroups().keySet();
		Map<String, String> kv = new HashMap<String, String>();
		for (String key : keys) {
			kv.put(key, group(key));
		}
	 ...
	 * the previous match: Each occurrence of <tt>${</tt><i>name</i><tt>}</tt> or
	 * <tt>$</tt><i>g</i> will be replaced by the result of evaluating the
	 * corresponding {@link #group(String) group(name)} or {@link #group(int)
	 * group(g)</tt>} respectively. For <tt>$</tt><i>g</i><tt></tt>, the first
	 * number after the <tt>$</tt> is always treated as part of the group reference.
	 * Subsequent numbers are incorporated into g if they would form a legal group
	 ...
	 * <tt>"foo"</tt>, for example, then passing the replacement string
	 * <tt>"$2bar"</tt> would cause <tt>"foobar"</tt> to be appended to the string
	 * buffer. A dollar sign (<tt>$</tt>) may be included as a literal in the
	 ...
	 * code, for example, writes <tt>one dog two dogs in the
	 ...
	 *
	 * @throws IllegalArgumentException
	 *             If the replacement string refers to a named-capturing group that
	 *             does not exist in the pattern
	 *
	 * @throws IndexOutOfBoundsException
	 *             If the replacement string refers to a capturing group that does
	 ...
	 * @throws IndexOutOfBoundsException
	 *             If start or end is less than zero, if start is greater than the
	 ...
	 *
	 * <p>
	 * Invoking this method with an argument of <tt>true</tt> will set this matcher
	 * to use <i>transparent</i> bounds. If the boolean argument is <tt>false</tt>,
	 ...
	 * <p>
	 * Invoking this method with an argument of <tt>true</tt> will set this matcher
	 * to use <i>anchoring</i> bounds. If the boolean argument is <tt>false</tt>,
